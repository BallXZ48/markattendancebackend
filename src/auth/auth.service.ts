...
export class AuthService {
  constructor(
    ...
    @InjectModel('Classsessions_S') private classSessionModel: Model<any>,
  ) {}
  ...
  async getProfileData(userId: string) {
    ...
    const { passwordHash, refreshTokenHash, ...result } = userObj;

    // 2. ดึงข้อมูลตารางเรียน และยัดใส่ result
    if (result.tableId) {
      const sessionData = await this.classSessionModel
        .findOne({ tableId: Number(result.tableId) })
        .exec();
      result.schedule = sessionData ? sessionData.schedule : [];
    } else {
      result.schedule = [];
    }
